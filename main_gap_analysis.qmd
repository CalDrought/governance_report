...
### Issue 2: PWSID's Containing NAs and Invalid values
...
This code returns a dataframe that shows which rows have invalid or `NA` `pwsid` values. Most observations within this returned dataframe consist of `NA` values, most likely stemming from not being reported correctly. However, some observations contain invalid `pwsid` values that ar enot the correct 9 character string length.
